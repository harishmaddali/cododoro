        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          ...
          {snapshot.recentCommits.map((c, i, a) => (
            <Fragment key={c.sha}>
              <CommitRow commit={c} />
              {i < a.length - 1 && (
                <div className="divider" style={{ marginLeft: 50 }} />
              )}
            </Fragment>
          ))}
        </div>
      ...
      {warning && (
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(circle at top right, var(--danger-soft), transparent 70%)",
            pointerEvents: "none",
          }}
        />
      )}
...
function RepoProgressRow({
  ...
  return (
    ...
    >
      ...
      >
        <div
          style={{ width: 8, height: 8, borderRadius: 2, background: repo.color }}
        />
      </div>
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          ...
          <span
            style={{ width: 3, height: 3, borderRadius: 999, background: "var(--fg-3)" }}
          />
...
function Mascot({ status }: { status: ReturnType<typeof deriveStatus> }) {
  const mood =
    status === "on-fire" ? "happy" : status === "in-progress" ? "neutral" : "sad";
