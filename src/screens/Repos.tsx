...
export function ReposScreen({ snapshot, days, onOpenRepo }: Props) {
  ...
  return (
    <div className="screen">
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              fontSize: 12,
              background:
                filter === f.id ? "rgba(57,216,120,0.12)" : "var(--bg-2)",
      ...
      >
        {shown.length === 0 && (
          <div className="card t-small">Nothing here yet.</div>
        )}
        {shown.map((r) => (
          <RepoCard
            key={r.nameWithOwner}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        >
          <span
            style={{ width: 6, height: 6, borderRadius: 999, background: repo.color }}
          />
        ...
        >
          <div>
            ...
            >
              {repo.today}
              {repo.goal > 0 && (
                <span style={{ color: "var(--fg-3)" }}>/{repo.goal}</span>
              )}
            </div>
